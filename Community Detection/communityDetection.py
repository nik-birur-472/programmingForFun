...
#
# The file `edges.txt.gz` indicates like relationships between facebook users. This was collected using snowball sampling: beginning with the user "Bill Gates", I crawled all the 
# people he "likes", then, for each newly discovered user, I crawled all the people they liked.
#
# I have clustered the resulting graph into communities, as well as recommend friends for Bill Gates.
#
#
...
def bfs(graph, root, max_depth):
    """
        ...
        In the doctests below, i first try with max_depth=5, then max_depth=2.
    ...
    for key in node2distances:
        if key[0] != root:
            for val in node2parents[key[0]]:
                t = (key[0], val)
                t = tuple(sorted(t))
                betweeness_edges[t] = betweeness_nodes_labels[key[0]] / (
                node2num_paths[val] * len(node2parents[key[0]]))
                betweeness_nodes_labels[val] = betweeness_nodes_labels[val] + (
                betweeness_nodes_labels[key[0]] / len(node2parents[key[0]]))
...
def download_data():
    """
        Download the data.
        """
    urllib.request.urlretrieve('PROVIDE YOUR URL HERE', 'FILENAME.txt.gz')
